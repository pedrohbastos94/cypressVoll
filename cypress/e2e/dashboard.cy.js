...
// código omitido
    ...
    context('Redirecionamente na página de dashboard', ()=>{
    ...
    context('Modal de cadastro de especialista', () => {
            ...
            it('Seleciona o botão checkbox "Atender por plano?" para visualizar os planos de saúde', () => {
                cy.visit('/dashboard')
                cy.contains('Cadastrar especialista').should('be.visible').click()
                cy.get('[type="checkbox"]').check()
                cy.get('form').find('input[type="checkbox"]').should('be.checked').and('not.be.disabled')
                cy.get('[type="checkbox"]').check(['Sulamerica', 'Unimed'])
            })

            it.only('Seleciona o botão checkbox "Atende por planos?" apos o preenchimento do formulário para visualizar os planos de saude', ()=>{
